...
nameThing = thingSaved #DO NOT DO THIS "works some of the time"
...
rowCookie<- rbind(animals, cookiesEaten)
```
...
```{r}
TV_shows <- c("The Office", "Parks and Recreation", "Veep", "Gilmore Girls")
TV_ratings <- c(8, 7.5, 7, 5)
```

3.  Make these vectors into a data frame.

```{r}
TV_show_ratings <- cbind.data.frame(TV_shows, TV_ratings)
```
